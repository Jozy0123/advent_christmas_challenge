...
class Signal:
    ...
    def __init__(self, singal_input):
        self.input = singal_input

    def __lt__(self, other):
        return real_iter(self.input, other.input)
...
if __name__ == '__main__':
    ...
    for i in pairs:
        lists = parse_tuple(i)
        for j in lists:
            lsts.append(Signal(singal_input=j))

    lsts.append(Signal([[2]]))
    lsts.append(Signal([[6]]))

    lsts.sort()

    index = 1
    for i in range(len(lsts)):
        if lsts[i].input == [[2]] or lsts[i].input == [[6]]:
            index *= (i + 1)
